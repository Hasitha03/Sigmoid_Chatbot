...
from src.utils.plot_heatmaps import plot_dual_calendar_heatmaps
...
def add_image_url_to_state(fig, state, azure_client):
    html_str = fig.to_html(full_html=True, include_plotlyjs='cdn')
    # Convert to BytesIO for uploading
    insights_html_bytes = BytesIO(html_str.encode('utf-8'))
    insights_html_filename = f"plotly-chart-{uuid.uuid4().hex}.html"
    insights_html_url = azure_client.upload_html_to_blob(file_bytes=insights_html_bytes,
                                                         filename=insights_html_filename, content_type="text/html")
    state['visual_outputs'].append(insights_html_url)
...
def process_bi_agent(state: AgentConversationState, llm, shipment_df, rate_card,azure_client):
    ...
    try:
        ...
        if state['reasoning_task'] == 'False' and result_df.shape != (1,2) and result_df.shape != (1,1):
            if final_results['figure'] is not None:
                ...
                add_image_url_to_state(fig, state, azure_client)

                # html_str = fig.to_html(full_html=True, include_plotlyjs='cdn')
                # # Convert to BytesIO for uploading
                # insights_html_bytes = BytesIO(html_str.encode('utf-8'))
                # insights_html_filename = f"plotly-chart-{uuid.uuid4().hex}.html"
                # insights_html_url = azure_client.upload_html_to_blob(file_bytes=insights_html_bytes,filename=insights_html_filename,content_type="text/html")
                # state['visual_outputs'].append(insights_html_url)
                # print(f"html url : {insights_html_url}")
                # print("html link stored in azur blob storage successfully..")
                #
                #
                # st.session_state.image.append(fig)
    ...
    return [HumanMessage(content=summary,name="InsightsAgent")]
    # return answer
...
#         add_image_url_to_state(shipment_window_image, state, azure_client)
#
...
#         fig = plot_dual_calendar_heatmaps(original_df, consolidated_df, save_path=heatmap_filename)
#         add_image_url_to_state(fig, state, azure_client)
#         # st.session_state.image.append(fig)
...
def Dynamic_CostOptimization_Agent(state: AgentConversationState, llm, shipment_df, rate_card, azure_client):
    ...
    try:
        ...
        shipment_window_image = create_shipment_window_vs_saving_plot(response_parameters["all_results"],
                                                                      response_parameters["best_params"])
        # st.session_state.image.append(shipment_window_image)
        add_image_url_to_state(shipment_window_image, state, azure_client)
        # shipment_win_filename = f"plotly-chart-{uuid.uuid4().hex}.png"
        # shipment_win_image_bytes = BytesIO()
        ...
        # shipment_window_image.write_image(shipment_win_image_bytes, format="png")
        # shipment_win_image_bytes.seek(0)
        # shipment_win_url = azure_client.upload_image_to_blob(shipment_win_image_bytes, shipment_win_filename,
        #                                                      "image/png")
        # shipment_window_image.write_image(shipment_win_filename)
        # state['visual_outputs'].append(shipment_win_url)
        consolidated_df = response_parameters['all_consolidated_shipments']
        ...
        # consolidated_df.to_csv("/Users/niteshkumar/Desktop/Nitesh/PERRIGO/Latest/Production/data/consolidated_df.csv")
        # original_df.to_csv("/Users/niteshkumar/Desktop/Nitesh/PERRIGO/Latest/Production/data/original_df.csv")
        ...
        consolidation_response = {
            'query': state["parameters"]["enriched_query"],
            'answer': response_parameters['final_response'].content if hasattr(response_parameters['final_response'],
                                                                               'content') else response_parameters[
                'final_response'],
            'timestamp': pd.Timestamp.now().strftime('%Y-%m-%d %H:%M:%S')
        }
        message = consolidation_response['answer']
        ...
        print(">>>Error in Dynamic Cost Optimization Agent", e)
        message = "Unable to extract answer for given query due to limitations in bot capabilities or ambiguity in the question. Please try rephrasing your question or providing more context."
    return [HumanMessage(content=message, name="DynamicCostOptimizationAgent")]
...
def Static_CostOptimization_agent(state: AgentConversationState,llm,shipment_df,rate_card,azure_client):
    ...
    fig = plot_dual_calendar_heatmaps(original_df, consolidated_df, save_path=heatmap_filename)
    # print("Got fig")
    # st.plotly_chart(fig, key="hfbejak")
    add_image_url_to_state(fig, state, azure_client)
    ...
    #     message = "Unable to extract answer for given query due to limitations in bot capabilities or ambiguity in the question. Please try rephrasing your question or providing more context."
    return [HumanMessage(content=message,name="StaticCostOptimizationAgent")]
...
def pallet_utilisation_optimisation_agent(state: AgentConversationState,llm,shipment_df,rate_card,azure_client):
    ...
    try:
        ...
        message = "Unable to extract answer for given query due to limitations in bot capabilities or ambiguity in the question. Please try rephrasing your question or providing more context."
    return [HumanMessage(content=message,name="PalletUtilisationOptimizationAgent")]
...
def drop_point_centralization_optimisation_agent(state: AgentConversationState,llm,shipment_df,rate_card,azure_client):
    ...
    return [HumanMessage(content=message,name="DropPointCentralizationOptimizationAgent")]
