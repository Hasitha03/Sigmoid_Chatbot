...
class AzureBlobStorage:
    ...
    def upload_html_to_blob(self, file_bytes, filename: str, content_type) -> str:
        ...
        blob_client.upload_blob(
            file_bytes.getvalue(),
            overwrite=True,
            content_settings=ContentSettings(content_type=content_type)  # Set HTML MIME type
        )
        ...
        sas_token = generate_blob_sas(
            ...
            container_name=self.container_name,
            blob_name=filename,
            account_key="DA4tYeyTFSTE8pIJEWBKt1ZPsqOY8xS1DXtpCGg2MhAhzUfuKDT6PYFN9E2Hg5BwlyPwBZIuNf+g+AStKYn7hA==",
            permission=BlobSasPermissions(read=True),
            expiry=datetime.utcnow() + timedelta(hours=1)
        )
