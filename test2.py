...
<style>
    ...
    .chat-container {
        ...
        margin-bottom: 2rem;
        margin-left: 1rem; /* Add some left margin to move away from sidebar */
...
def render_welcome_message():
    ...
    if st.session_state.show_welcome and not st.session_state.chat_history:
        ...
        <div class="questions-section">
            <div class="questions-title">🚀 Quick Start Questions</div>
        </div>
        """, unsafe_allow_html=True)
...
def display_chart(chart, chart_index: int, message_index: int, chart_type: str = "user"):
    ...
    try:
        ...
        # Check if it's an image URL or path
            ...
            print(f"printing chart from streamlit : {chart}")
            if chart.startswith(('http://', 'https://', 'data:')):
                # Fetch HTML content from the URL
                res = requests.get(chart)
                # Display it inside the app
                if res.status_code == 200:
                    components.html(res.text, height=600, scrolling=True)
            else:
                st.error("Failed to load interactive chart.")
...
def render_chat_interface():
    ...
    for i, message in enumerate(st.session_state.chat_history):
        ...
        text_streamed = message['text_streamed']

        if role == "user":
            if not text_streamed:
                placeholder = st.empty()
                # Initialize streamed content
                streamed = ""
                    ...
                    placeholder.markdown(f"""
                        <div style="display: flex; align-items: flex-start; justify-content: flex-end; margin-bottom: 10px;">
                            <div class="message-container user-message-container">
                                <div class="user-message" style="background-color: #DCF8C6; padding: 10px 14px; border-radius: 10px; max-width: 70%;">
                                    {streamed}▌
                                    {f'<br><small style="opacity: 0.8;">📅 {timestamp}</small>' if timestamp else ''}
                                </div>
                            </div>
                            <span class="user-icon" style="margin-left: 8px; font-size: 40px; margin-top: 5px;">👱‍♂️</span>
                        </div>
                    """, unsafe_allow_html=True)
                    time.sleep(0.01)
            ...
            if content:
                ...
                if not text_streamed:
                    ...
                    for char in agent_content:
                        streamed += char
                        placeholder.markdown(f"""
                            <div style="display: flex; align-items: flex-start; justify-content: flex-start; margin-bottom: 10px;">
                                <span class="bot-icon" style="margin-right: 8px; font-size: 40px; margin-top: 5px;">🤖</span>
        ...
        st.session_state.chat_history[i] = {
            "role": role,
            "content": content,
            "img_list": img_list,  # Store current plots with user message
            "text_streamed": True,
            "timestamp": timestamp
        }
...
def render_follow_up_questions():
    ...
    if st.session_state.follow_up:
        ...
        cols = st.columns(min(len(st.session_state.follow_up), 1))
...
def main_app():
    ...
    st.sidebar.markdown("---")
    st.sidebar.markdown(
        "<div style='text-align: center; color: #4a5568; padding: 0rem; margin-bottom: 0px; font-weight: 500;'>"
